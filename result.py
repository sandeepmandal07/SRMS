...
class resultClass:
    ...
    #=====Variables====
       ...
       #====button====
       btn_add=Button(self.root,text="Submit",font=("times new roman",15),bd=5,relief=RIDGE,bg="lightgreen",activebackground="lightgreen",cursor="hand2",command=self.add).place(x=300,y=420,width=120,height=35)
       btn_clear=Button(self.root,text="Clear",font=("times new roman",15),relief=RIDGE,bd=5,bg="lightgray",activebackground="lightgray",cursor="hand2",command=self.clear).place(x=430,y=420,width=120,height=35)

       #====image====
       self.bg_img = Image.open("images/result.jpeg")
       self.bg_img = self.bg_img.resize((500, 300), Image.BICUBIC)
       self.bg_img = ImageTk.PhotoImage(self.bg_img)
       self.bg_img_label = Label(self.root, image=self.bg_img).place(x=650, y=100)
